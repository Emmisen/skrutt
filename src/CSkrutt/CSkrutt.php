 ...
 * @package SkruttCore
 */
class CSkrutt implements ISingleton {
	...
	protected function __construct() {
		...
		// include the site specific config.php and create a ref to $sk to be used by config.php
		$sk = &$this;
    require(SKRUTT_SITE_PATH.'/config.php');
  ...
  /**
	 ...
	 * @return CSkrutt The instance of this class.
	 */
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= SKRUTT_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = SKRUTT_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $sk->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $sk
    ...
    $sk = &$this;
    // First the default Skrutt themes/functions.php
  ...
  /**
   * Draw HTML for a menu defined in $sk->config['menus'].
   *
